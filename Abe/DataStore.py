...
CHAIN_CONFIG = [
    {"chain":"Globe",
     "code3":"GLB", "address_version":"\x00", "magic":"\xf9\xbe\xb4\xd9"},
    {"chain":"Testnet",
...
# Regex to match a pubkey hash ("Globe address transaction") in
# txout_scriptPubKey.  Tolerate OP_NOP (0x61) at the end, seen in Globe
# 127630 and 128239.
...
class DataStore(object):
    ...
    """
    Globe data storage class based on DB-API 2 and SQL1992 with
    ...
    def __init__(store, args):
        ...
        args.datadir names Globe data directories containing
    ...
    def _init_datadirs(store):
        ...
        # practise, because abe.py sets ~/.globe as default datadir.
...
# in the original Globe source as `pchMessageStart'.
"""CREATE TABLE magic (
...
# A block of the type used by Globe.
"""CREATE TABLE block (
...
# A transaction of the type used by Globe.
"""CREATE TABLE tx (
...
# A public key for sending globes.  PUBKEY_HASH is derivable from a
# Globe or Testnet address.
"""CREATE TABLE pubkey (
...
#    def _is_descended_from(store, block_id, ancestor_id):
    ...
    def script_to_pubkey_id(store, script):
        ...
        # Not a standard Globe script as of 2011-08-23.  Namecoin operation?
    ...
    def catch_up_rpc(store, dircfg):
        ...
        getblockhash, getblock, and getrawtransaction.  Globed v0.8
        ...
        conffile = dircfg.get("conf",
                              os.path.join(dircfg['dirname'], "globe.conf"))
        try:
        ...
        rpcport     = conf.get("rpcport",
                               "18682" if "testnet" in conf else "8682")
        ...
        try:
            ...
            store.log.debug("globed %s not supported", e.method)
            return False
    ...
    def catch_up_dir(store, dircfg):
        def open_blkfile(number):
            ...
            try:
                ...
                # Early globed used blk0001.dat to blk9999.dat.
        ...
        while filenum == dircfg['blkfile_number']:
            ...
            if magic[0] == "\0":
                if filenum > 99999 and magic == "\0\0\0\0":
                    # As of Globe 0.8, files often end with a NUL span.
