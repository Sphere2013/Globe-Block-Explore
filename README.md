Abe: a free block chain browser for Globe-based currencies.
https://github.com/globe-abe/globe-abe
...
This software reads the Globe block file, transforms and loads the
data into a database, and presents a web interface similar to Globe
Block Explorer, http://blockexplorer.com/.

Abe draws inspiration from Globe Block Explorer (BBE) and seeks some
...
Abe works with files created by the original (Satoshi) Globe client.
You will need a copy of the block files (blk0001.dat, blk0002.dat,
etc. in your Globe directory or its blocks/ subdirectory).  You may
let Abe read the block files while Globe runs, assuming Globe only
appends to the file.  Prior to Globe v0.8, this assumption seemed
safe.  Abe may need some fixes to avoid skipping blocks while current
and future Globe versions run.
...
the main GLB block chain as of 2013.  However, if you use a persistent
...
the default Globe directory, specify "datadir" in abe.conf, e.g.:

    datadir = /home/globe/.namecoin
...
Forum thread: https://globetalk.org/index.php?topic=22785.0
Newbies: https://globetalk.org/index.php?topic=51139.0

Donations appreciated: 1PWC7PNHL1SgvZaN7xEtygenKjWobWsCuf (GLB)
